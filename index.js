...
async function run() {
    try {
        ...
        const menuCollection = client.db('foodvibesDB').collection('menu');
        const reviewCollection = client.db('foodvibesDB').collection('reviews');
...
app.listen(port, () => {
    console.log('the server is running form', port)
})
